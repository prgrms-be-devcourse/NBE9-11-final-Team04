...
class ExpertVerifyServiceTest {
    ...
    void verify_국가자격_파일없음_INVALID_INPUT_예외() {
        given(userRepository.findById(1L)).willReturn(Optional.of(user()));
        given(expertProfileRepository.findByUserId(1L)).willReturn(Optional.empty());

        assertThatThrownBy(() -> expertVerifyService.verify(1L, nationalRequest(), null))
                .isInstanceOf(CustomException.class)
                .extracting(e -> ((CustomException) e).getErrorCode())
    ...
    @Test
    @DisplayName("NATIONAL_QUALIFICATION 검증 시 파일이 업로드되고 보류 상태 프로필이 저장된다")
    void verify_국가자격검증_파일업로드_PENDING_VERIFICATION프로필저장() {
        MultipartFile file = certFile();
        given(userRepository.findById(1L)).willReturn(Optional.of(user()));
        given(expertProfileRepository.findByUserId(1L)).willReturn(Optional.empty());
        given(externalVerifyClient.verify(any(), any(boolean.class))).willReturn(true);
        given(appealStorageClient.upload(file)).willReturn("expert/qualification/uuid.pdf");
        given(expertProfileRepository.save(any())).willAnswer(i -> i.getArgument(0));
