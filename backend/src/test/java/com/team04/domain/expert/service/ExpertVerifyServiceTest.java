...
class ExpertVerifyServiceTest {
    ...
    void verify_이미검증된프로필존재_DUPLICATE_EXPERT_PROFILE_예외() {
        ...
        assertThatThrownBy(() -> expertVerifyService.verify(1L, businessRequest()))
    ...
    @Test
    @DisplayName("NATIONAL_QUALIFICATION 검증 시 fileUrl이 없으면 INVALID_INPUT 예외가 발생한다")
    void verify_국가자격_fileUrl없음_INVALID_INPUT_예외() {
        given(userRepository.findById(1L)).willReturn(Optional.of(user()));
        given(expertProfileRepository.findByUserId(1L)).willReturn(Optional.empty());

        assertThatThrownBy(() -> expertVerifyService.verify(1L, nationalRequestWithoutFile()))
                .isInstanceOf(CustomException.class)
                .extracting(e -> ((CustomException) e).getErrorCode())
    ...
    void verify_사업자검증성공_ACTIVE프로필저장() {
        ...
        var response = expertVerifyService.verify(1L, businessRequest());
    ...
    void verify_국가자격검증_PENDING_VERIFICATION프로필저장() {
        ...
        var response = expertVerifyService.verify(1L, nationalRequest());
