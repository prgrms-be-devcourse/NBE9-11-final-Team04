...
public class OpenAiVerificationService {
    ...
    private String buildPrompt(VerificationRequest request) {
        return "제목: " + request.title() + "\n"
                ...
                + "아래 3가지 항목을 판단하라. 탐지 결과는 참고용이며 단독으로 반려 또는 보완 사유가 되지 않는다.\n"
                + "decision은 항상 INFO로 응답한다.\n\n"
                + "1. 과대광고(EXAGGERATED_ADVERTISEMENT) 0~10점:\n"
                + "   - 10점: 과장 표현 없음, 검증 가능한 수치 사용\n"
                + "   - 5점: 일부 과장 표현 있으나 전반적으로 합리적\n"
                + "   - 0점: '100% 보장', '확정 수익' 등 명백한 과대광고\n\n"
                + "2. 유사서비스(SIMILAR_SERVICE) 0~10점:\n"
                + "   - 10점: 명확한 차별점 있음\n"
                + "   - 5점: 유사 서비스 있으나 차별점 설명됨\n"
                + "   - 0점: 동일한 서비스가 이미 존재하고 차별점 없음\n\n"
                + "3. 마일스톤 구체성(MILESTONE_SPECIFICITY) 0~20점:\n"
                + "   - 20점: 목표/기대결과/예정일 모두 구체적\n"
                + "   - 10점: 일부 항목이 모호함\n"
                + "   - 0점: 전반적으로 추상적이고 측정 불가";
    }
