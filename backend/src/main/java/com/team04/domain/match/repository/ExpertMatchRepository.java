...
public interface ExpertMatchRepository extends JpaRepository<ExpertMatch, Long> {
    ...
    // 전문가 본인의 매칭 목록 조회
    @Query("SELECT m FROM ExpertMatch m JOIN FETCH m.expertProfile ep WHERE ep.user.id = :userId")
    List<ExpertMatch> findAllByUserId(@Param("userId") Long userId);

    // 매칭 단건 조회 (권한 검증 포함)
    @Query("SELECT m FROM ExpertMatch m JOIN FETCH m.expertProfile ep WHERE m.id = :matchId AND ep.user.id = :userId")
