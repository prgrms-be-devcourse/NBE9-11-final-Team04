...
export default function ReceivedDisputesPage() {
  const [page, setPage] = useState(0)

  const { data, isLoading } = useQuery({
    queryKey: ['disputes', 'received', page],
    queryFn: () => disputesApi.getReceivedDisputes(page, 10),
  })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {items.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {items.map((d) => {
            const needsAppeal = d.status === 'RECEIVED'
            return (
              <Link
                key={d.id}
                href={`/disputes/${d.id}`}
              ...
              >
                <div style={{
                  background: '#fff',
                  border: `1.5px solid ${needsAppeal ? '#fca5a5' : 'var(--border)'}`,
                  borderRadius: '12px',
                  padding: '16px 20px',
                  display: 'flex',
                  alignItems: 'center',
                  ...
                  onMouseEnter={(e) => { e.currentTarget.style.borderColor = needsAppeal ? '#ef4444' : 'var(--brand)' }}
                  onMouseLeave={(e) => { e.currentTarget.style.borderColor = needsAppeal ? '#fca5a5' : 'var(--border)' }}
                >
                  <div style={{
                    width: '44px',
                    height: '44px',
                    borderRadius: '8px',
                    background: needsAppeal ? '#fff5f5' : 'var(--bg-alt)',
                    display: 'flex',
                    alignItems: 'center',
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', marginTop: '8px' }}>
          <button
            ...
            style={{
              ...
              color: page === 0 ? 'var(--fg-muted)' : 'var(--fg)', fontSize: '14px',
            }}
          >
            이전
          </button>
          ...
          <button
            ...
            style={{
              ...
              color: page >= totalPages - 1 ? 'var(--fg-muted)' : 'var(--fg)', fontSize: '14px',
            }}
          >
            다음
          </button>
